...
app.whenReady().then(async () => {
  try {
    ...
    // Create and set AI Agent Tester working directory in Documents
    const documentsPath = app.getPath('documents');
    const agentWorkingDir = path.join(documentsPath, 'AI Agent Tester');

    try {
      await fs.mkdir(agentWorkingDir, { recursive: true });
      console.log(`📁 AI Agent Tester working directory: ${agentWorkingDir}`);
      process.env.AGENT_WORKING_DIR = agentWorkingDir;

      // Create instructions.txt file if it doesn't exist
      const instructionsPath = path.join(agentWorkingDir, 'instructions.txt');
      try {
        await fs.access(instructionsPath);
        console.log('📄 instructions.txt already exists');
      } catch (error) {
        // File doesn't exist, create it
        const instructionsContent = `AI Agent Tester - Working Directory
=====================================

This folder was automatically created by AI Agent Tester.

PURPOSE:
This is the working directory where AI agents (Gemini, Claude, ChatGPT, etc.)
operate when you interact with them through the AI Agent Tester application.

WHAT THIS MEANS:
- When you ask an agent to create, read, or modify files, they will do so in this folder
- Agents can see and access files in this directory
- Any files created by agents will appear here

TRY IT OUT:
You can test this by asking an agent questions like:
- "What files do you see in the current directory?"
- "Can you read the instructions.txt file and tell me what it says?"
- "Create a new file called test.txt with some content"
- "List all files in this folder"

The agents should be able to interact with this folder and its contents!

---
Generated by AI Agent Tester
`;
        await fs.writeFile(instructionsPath, instructionsContent, 'utf-8');
        console.log('✅ Created instructions.txt');
      }
    } catch (error) {
      console.error('Failed to create AI Agent Tester directory:', error);
      // Fall back to Documents folder if creation fails
      process.env.AGENT_WORKING_DIR = documentsPath;
    }
